...
async def start(client, message):
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        try:
            ...
            protect = "/pbatch" if PROTECT_CONTENT else "batch"
        diff = int(l_msg_id) - int(f_msg_id)
        async for msg in client.iter_messages(int(f_chat_id), offset_id=int(f_msg_id), limit=diff):
            title = msg.caption or ""
            if BATCH_FILE_CAPTION:
                try:
                    f_caption = BATCH_FILE_CAPTION.format(mention=message.from_user.mention, file_name=title, file_size=get_size(msg.file.size), file_caption=title)
                except Exception as e:
                    logger.exception(e)
                    f_caption = title
            else:
            ...
            if not title:
                title = "Unknown"
            try:
                await client.send_cached_media(chat_id=message.from_user.id, file_id=msg.file_id, caption=f_caption, protect_content=protect)
            except FloodWait as e:
                await asyncio.sleep(e.value)
                await client.send_cached_media(chat_id=message.from_user.id, file_id=msg.file_id, caption=f_caption, protect_content=protect)
            await asyncio.sleep(1)
